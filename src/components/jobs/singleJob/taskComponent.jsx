...
const DisplayTasks = ({ jobId }) => {
  ...
  function EditableControls({ isComplete }) {
    const {
      isEditing,
  ...
  const onComplete = async (taskId, isComplete, task) => {
    ...
    completiontoast({
      title: !isComplete ? "Completion" : "Ongoing",
      description: task,
    ...
    return (
      <HStack marginBottom={2}>
        <Text marginLeft={2} fontSize="xl">
          ...
          >
            <HStack>
              <EditableControls isComplete={task.isComplete} />
              <EditablePreview
        ...
        <Spacer />
        <IconButton
          onClick={() => onComplete(task.id, task.isComplete, task.task)}
          icon={
            task.isComplete ? (
